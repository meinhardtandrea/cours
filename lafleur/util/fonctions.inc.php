...
function getErreursSaisieCommande( $nom, $rue, $ville, $cp, $mail) {
    ...
    if( $nom == "")   { $lesErreurs[] = "Il faut saisir le champ Nom Prénom"; }
    if( $rue=="")     { $lesErreurs[] = "Il faut saisir le champ Rue"; }
    if( $ville == "") { $lesErreurs[] = "Il faut saisir le champ Ville"; }
    if( $cp == "")    { $lesErreurs[] = "Il faut saisir le champ Code postal"; } else { if( !estUnCp( $cp))    { $lesErreurs[] = "Erreur de code postal"; }}
    if($mail=="")     { $lesErreurs[] = "Il faut saisir le champ Mail"; }        else { if( !estUnMail($mail)) { $lesErreurs[] = "Erreur de mail"; }}
