...
WHERE news_mod_id = '$mod_id' AND news_activation = 'active' ORDER BY news_date DESC, news_heure DESC LIMIT 0,4 ";
...
echo '<p class="contenthover"><a href="news-'.$page.'-'.$news_id.'.html" class="mybutton">Lire la suite</a></p>'."\n";
...
$sql_a = "SELECT * FROM " .$table_news. "
LEFT JOIN " .$table_users. " ON ".$table_news.".news_author = ".$table_users.".user_id
WHERE news_mod_id = '$mod_id' AND news_activation = 'active' ORDER BY news_date DESC, news_heure ";
$req_a = mysql_query($sql_a) or die('Erreur SQL !'.$sql_a.'<br>'.mysql_error());
$res_a = mysql_num_rows($req_a);

if($res_a >= 5){
$sql_af = "SELECT * FROM " .$table_news. "
LEFT JOIN " .$table_users. " ON ".$table_news.".news_author = ".$table_users.".user_id
WHERE news_mod_id = '$mod_id' AND news_activation = 'active' ORDER BY news_date DESC, news_heure DESC LIMIT 4,5 ";
$req_af = mysql_query($sql_af) or die('Erreur SQL !'.$sql_af.'<br>'.mysql_error());
$res_af = mysql_num_rows($req_af);
if($res_af){
$row_af = mysql_fetch_array($req_af);
$news_id = $row_af['news_id'];
echo '<p align="right"><a href="news-'.$page.'-'.$news_id.'.html" class="mybutton">Consulter les archives</a> >></p>'."\n";
}
}
...
echo '<form action="mail.php?position='.$rub_position.'&mod_id='.$mod_id.'" method="post">'."\n";
...
echo '<p><textarea cols="40" id="msg" name="msg" rows="5" ></textarea></p>'."\n";
...
if($mod_texte){
echo '</td>'."\n";
echo '<td valign="top" width="300">'.$mod_texte.'</td>'."\n";
echo '</tr>'."\n";
echo '</table>'."\n";
}
...
echo '<table width="100%">'."\n";
echo '<tr>'."\n";
echo '<td valign="top" width="300"><img src="files/images/'.$mod_imgb.'" height="80"></td>'."\n";
echo '<td align="center" valign="middle" width="300"><span class="inscrip"><a href="inscription.html" class="mybutton">Inscrivez-vous</a></span></td>'."\n";
echo '</tr>'."\n";
echo '</table>'."\n";
}
...
/*
$sql_ct = "SELECT COUNT( DISTINCT `piwik_log_link_visit_action`.`idvisit` ) AS unique_visits FROM piwik_log_action, piwik_log_link_visit_action 
WHERE `piwik_log_link_visit_action`.`idaction_url`=`piwik_log_action`.`idaction` and `piwik_log_link_visit_action`.`idsite`=1 and 
`piwik_log_link_visit_action`.`idaction_name` IS NOT NULL and  `piwik_log_action`.`name` like '%Fiches/CircuitsCourts.php%'";
$req_ct = mysql_query($sql_ct) or die('Erreur SQL !'.$sql_ct.'<br>'.mysql_error());
$res_ct = mysql_num_rows($req_ct);

if($res_ct){
$row_ct = mysql_fetch_array($req_ct);
echo '<div id="fiches_over"><p>'.$row_ct['unique_visits'].' <img src="images/eye.png"></p></div>'."\n";
}
*/
